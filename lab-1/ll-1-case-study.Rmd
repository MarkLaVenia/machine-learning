...
#setwd("C:/Users/MarkLaVenia/GitHub/machine-learning")
```
...
-   3,793 obs. of 5 variables

-   4 vars are dbl class and 1 is chr
...
```{r}
train_prop <- count(data_train) / count(d)
...
-   The model correctly predicted the classification 87% of the time.

-   This seems like a pretty high level of correct prediction, given the
...
-   To the extent that understanding incorrect prediction is important,
    it might be insightful to inspect whether it was usually wrong
    because it over predicted TS or SB, or was it roughly incorrect on
    each evenly. Also some understanding of base rates of each
    classification in the coded data could inform our interpretation of
    incorrectly predicted classifications.

-   roc_auc: Area under the receiver operator curve of 94% is also a
    fairly diagnostic indicator of the model's performance.
